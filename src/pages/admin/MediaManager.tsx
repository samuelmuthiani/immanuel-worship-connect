...
import { Loader2, Plus, Pencil, Trash2, Save, X, Image, Video } from 'lucide-react';
      ...
      <Tabs defaultValue="photos" className="space-y-4">
        <TabsList>
          <TabsTrigger value="photos" className="flex items-center gap-2">
            <Image className="h-4 w-4" /> Photos ({photos?.length || 0})
          </TabsTrigger>
          <TabsTrigger value="videos" className="flex items-center gap-2">
            <Video className="h-4 w-4" /> Videos ({videos?.length || 0})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="photos" className="space-y-4">
          ...
          {isEditingPhoto ? (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{currentPhoto.id ? 'Edit Photo' : 'Add Photo'}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label>Image URL</Label>
                  <Input value={currentPhoto.url || ''} onChange={e => setCurrentPhoto({ ...currentPhoto, url: e.target.value })} placeholder="https://..." />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label>Title</Label>
                    <Input value={currentPhoto.title || ''} onChange={e => setCurrentPhoto({ ...currentPhoto, title: e.target.value })} placeholder="Photo title" />
                  </div>
                  <div>
                    <Label>Category</Label>
                    <Input value={currentPhoto.category || ''} onChange={e => setCurrentPhoto({ ...currentPhoto, category: e.target.value })} placeholder="e.g. Worship, Events" />
                  </div>
                </div>
                <div>
                  <Label>Description</Label>
                  <Textarea value={currentPhoto.description || ''} onChange={e => setCurrentPhoto({ ...currentPhoto, description: e.target.value })} rows={2} />
                </div>
            ...
            <div className="grid gap-3">
                ...
                <Card key={photo.id}>
                  <CardContent className="p-4 flex justify-between items-center">
                    ...
                    <div className="flex gap-1 ml-2">
                      <Button variant="ghost" size="icon" onClick={() => { setCurrentPhoto(photo); setIsEditingPhoto(true); }}>
              ...
              {photos?.length === 0 && <p className="text-center text-muted-foreground py-8">No photos yet.</p>}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="videos" className="space-y-4">
          ...
          {isEditingVideo ? (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{currentVideo.id ? 'Edit Video' : 'Add Video'}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label>Video URL (YouTube/Vimeo embed URL)</Label>
                  <Input value={currentVideo.url || ''} onChange={e => setCurrentVideo({ ...currentVideo, url: e.target.value })} placeholder="https://www.youtube.com/embed/..." />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label>Title</Label>
                    <Input value={currentVideo.title || ''} onChange={e => setCurrentVideo({ ...currentVideo, title: e.target.value })} placeholder="Video title" />
                  </div>
                  <div>
                    <Label>Category</Label>
                    <Input value={currentVideo.category || ''} onChange={e => setCurrentVideo({ ...currentVideo, category: e.target.value })} placeholder="e.g. Sermons, Worship" />
                  </div>
                </div>
                <div>
                  <Label>Description</Label>
                  <Textarea value={currentVideo.description || ''} onChange={e => setCurrentVideo({ ...currentVideo, description: e.target.value })} rows={2} />
                </div>
                <div className="flex justify-end gap-2">
              ...
              {videos?.map(video => (
                <Card key={video.id}>
                  <CardContent className="p-4 flex justify-between items-center">
                    ...
                    <div className="flex gap-1 ml-2">
                      <Button variant="ghost" size="icon" onClick={() => { setCurrentVideo(video); setIsEditingVideo(true); }}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => {
                        if (confirm('Delete this video?')) deleteVideoMutation.mutate(video.id);
                      }}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
